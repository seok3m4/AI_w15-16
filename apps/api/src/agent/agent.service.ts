  ...
  async ask(question: string): Promise<AgentAnswer> {
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        ...
        content:
          ...
          '큰 제목(#)은 쓰지 말고, **추천 코스**, **추천 이유**, **일차별 일정**, **주의할 점**처럼 굵은 소제목을 사용해라. ' +
          '한 문단은 2~3문장을 넘기지 말고, 일정은 번호 목록이나 짧은 bullet 목록으로 정리해라.',
      },
    ...
    if (!answer) {
      messages.push({
        role: 'user',
        content:
          '지금까지 찾은 정보로 질문에 대한 답변을 한국어 Markdown으로 읽기 좋게 정리해 줘.',
      });
  ...
  private async buildExternalFallbackAnswer(
    ...
    return this.openai.chat(
      '너는 여행 코스 공유 게시판의 AI 도우미야. 게시판에서 관련 코스를 찾지 못한 상황이므로, 아래 외부 장소 검색 결과만 참고해서 답해. ' +
        ...
        '장소 이름은 검색 결과에 있는 이름만 사용하고, 없는 세부 정보는 지어내지 마. ' +
        '한국어 Markdown으로 간결하게 답하고, **외부 참고**, **추천 장소**, **주의할 점** 같은 굵은 소제목을 사용해라.',
      `사용자 질문: ${question}\n\n외부 장소 검색 결과:\n${placeContext}`,
    );
  }
}
