...
// 경유지를 1일차/2일차 단위로 묶어서 실제 여행 일정처럼 보여준다.
function groupPlacesByDay<T extends { day?: number }>(places: T[]) {
  const groups = new Map<number, Array<T & { courseOrder: number }>>();

  for (const [index, place] of places.entries()) {
    const day = place.day ?? 1;
    groups.set(day, [
      ...(groups.get(day) ?? []),
      { ...place, courseOrder: index + 1 },
    ]);
  }
            ...
            {post.places.length > 0 && (
              ...
              >
                ...
                <div className="course-days">
                  {groupPlacesByDay(post.places).map(([day, places]) => (
                    <section className="course-day" key={day}>
                      <h3>{day}일차</h3>
