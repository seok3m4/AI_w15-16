...
검색 결과가 없으면 동일하게 `200 OK`를 반환하고 `results`는 빈 배열(`[]`)이다.
...
MCP Server는 MVP에서 `POST /mcp` JSON-RPC endpoint로 제공한다. 외부 MCP client는 앱 access token을 재사용하지 않고 사용자별 scoped token을 `Authorization: Bearer <mmt_mcp_...>`로 전달한다. REST API 명세에서는 도구 경계, 최소 입력/출력, 권한 규칙을 고정한다.
...
- scope는 `memory:read`, `capsule:read`, `friend_memory:read`로 시작한다.
- scoped token 원문은 발급 응답에서 한 번만 반환하고 서버에는 HMAC-SHA256 hash와 secret reference만 저장한다.
- MCP Server 관리 API는 `GET /api/v1/mcp/tools`, `GET /api/v1/mcp/connections`, `POST /api/v1/mcp/server-credentials`, `DELETE /api/v1/mcp/connections/{connectionId}`, `GET /api/v1/mcp/call-logs`를 제공한다.
- 친구 데이터를 반환하는 tool은 accepted 친구 관계와 친구 AI 공유 동의를 모두 검증한다.
- 친구 데이터가 포함된 출력은 `sources`에 원본 게시글 참조를 포함한다.
- `mcp_call_logs`에는 argument key, status, error code 등 마스킹된 요약만 저장하고 token, secret, raw prompt, provider 원문은 저장하지 않는다.
...
- MCP Client(Notion) OAuth/official MCP 연동 세부는 T4 Notion Client 구현에서 확정한다.
