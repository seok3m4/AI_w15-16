...
- 배포 전략은 Track A 로컬 Docker 실행과 Track B AWS 배포로 분리한다.
- Track A는 Docker Compose 기반으로 PostgreSQL/pgvector, Spring Boot, FastAPI, React를 로컬에서 함께 실행하는 1차 데모 경로다.
- Track B의 공식 AWS 배포 기준은 ECS Fargate, RDS PostgreSQL/pgvector, S3/CloudFront, ALB/ACM HTTPS로 둔다.
- AWS 배포는 Track A 로컬 Docker smoke check가 통과한 뒤 진행한다.
- 운영 secret, JWT signing key, refresh token pepper, 외부 AI/MCP provider token은 AWS Secrets Manager와 KMS 기반 관리 대상으로 둔다.
- 운영 환경에서는 HTTPS, 제한된 CORS origin, `Secure` refresh token cookie를 필수로 한다.
- Spring Boot와 FastAPI는 배포 health check endpoint를 제공해야 한다.
- RDS는 저장소 암호화, 자동 백업, private network 접근을 기본 운영 요구사항으로 둔다.
- CloudWatch Logs에서 backend, ai-server, ECS task 로그를 확인할 수 있어야 한다.
...
- Track A 로컬 Docker 환경에서 frontend, backend, ai-server, PostgreSQL/pgvector가 함께 실행된다.
- Track A 로컬 Docker 환경에서 API health check, AI server health check, 게시글 작성, memory 상태 조회, Memory Search, AI 요약 smoke scenario가 성공한다.
- Track B AWS MVP 데모 환경에서 HTTPS 프론트 접속, API health check, AI server health check가 성공한다.
- AWS MVP 데모 환경에서 회원가입, 로그인, 게시글 작성, memory 상태 조회, Memory Search, AI 요약 smoke scenario가 성공한다.
- ECS service 재배포 이후에도 RDS에 저장된 사용자/게시글 데이터가 유지된다.
- CloudWatch Logs에서 backend와 ai-server의 주요 요청/오류 로그를 확인할 수 있다.
- 직전 ECS task definition 또는 직전 frontend build artifact로 수동 롤백할 수 있다.
...
6. 로컬 Docker 실행 설계: Docker Compose 기준 실행 구조, 환경 변수, health check, smoke check 확정
7. AWS 배포 설계: ECS/RDS/S3/CloudFront 기준 배포 구조, secret, smoke check, rollback 절차 확정
...
- 로컬 Docker에서 AI provider를 mock으로 둘지 실제 provider까지 연결할지 구현 단계에서 확정한다.
- AWS 운영 도메인, Route 53 사용 여부, NAT Gateway 비용 허용 범위는 배포 직전에 확정한다.
- Terraform/CDK 같은 IaC 도입은 MVP 데모 이후 운영 베타 단계에서 검토한다.
