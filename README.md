...
사용자는 야구 경기 리뷰, 선수 분석, 팀 이슈, 뉴스 브리핑 게시글을 작성하고 댓글, 태그, 검색, 페이지네이션으로 게시판을 사용할 수 있습니다. AI 기능은 기존 게시글 기반 유사 글 추천, KBO 공식 경기 데이터 조회, 외부 뉴스/URL 브리핑, 경기 리뷰 작성 보조를 제공합니다.

추가로 KBO 공식 뉴스 목록을 별도 페이지에서 확인하고 원문 URL을 복사할 수 있으며, 글쓰기 화면에서는 기존 태그를 확인해 클릭만으로 태그 입력에 추가할 수 있습니다.
...
- MCP 기반 KBO 공식 경기 데이터 조회 / 야구 뉴스 키워드 검색 / 외부 URL 브리핑
- AI Agent 기반 경기 리뷰 작성 도우미

### UI/UX

- KBO 공식 홈페이지 분위기를 참고한 네이비/레드 기반 레이아웃
- 상단 헤더, 게시글 카드, 태그 사이드바, AI 도구 패널 디자인 통일
- 홈 화면 전광판형 소개 영역
- 뉴스 페이지 썸네일 카드형 목록
...
flowchart LR
    ...
    MCP --> External["KBO 공식 일정/결과 / Google News RSS / External URL"]
...
KBO 공식 홈페이지의 최신 뉴스 목록을 별도 페이지(`/news`)에서 확인할 수 있도록 구현했습니다. 사용자는 게시판 안에서 KBO 뉴스를 훑어보고, 필요한 기사 원문 URL을 복사해 글쓰기나 MCP URL 브리핑에 활용할 수 있습니다.
...
### MCP: KBO 경기 데이터 / 야구 뉴스 / 외부 URL 브리핑
...
→ MCP tool 실행
→ KBO 공식 일정/결과, 외부 뉴스 RSS 또는 URL 데이터 수집
...
3. 야구 경기 리뷰 게시글을 작성하고 태그를 등록합니다.
4. 게시글 목록에서 여러 태그를 동시에 선택해 필터링합니다.
5. 게시글 상세 화면에서 RAG 유사 게시글 추천과 요약을 확인합니다.
6. 홈 화면에서 날짜와 팀을 선택해 KBO 공식 경기 일정/결과를 조회합니다.
7. 뉴스 페이지에서 KBO 공식 뉴스를 확인하고 원문 URL을 복사합니다.
8. MCP 패널에서 야구 키워드 또는 뉴스 URL로 브리핑을 생성합니다.
9. Agent 패널에 짧은 경기 메모를 입력해 추천 제목, 태그, 리뷰 초안, 체크리스트를 생성합니다.
10. 생성된 초안을 바탕으로 게시글을 작성하고 댓글을 남깁니다.
...
- KBO 경기 데이터와 게시글 작성 화면을 연결한 리뷰 자동 채우기
- KBO 뉴스 URL을 글쓰기 화면 또는 MCP URL 브리핑 입력으로 바로 넘기는 흐름
- 관리자용 AI 모더레이션 기능
