...
class JdbcMemoryVectorSearchRepositoryTest {
    ...
    @Test
    void searchFriendRestrictsCandidatesToAcceptedFriendSource() throws Exception {
        JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
        JdbcMemoryVectorSearchRepository repository = new JdbcMemoryVectorSearchRepository(jdbcTemplate);
        when(jdbcTemplate.query(
                        anyString(),
                ...
                .thenAnswer(invocation -> {
                    @SuppressWarnings("unchecked")
                    RowMapper<MemoryVectorSearchCandidate> mapper = invocation.getArgument(1);
                    return List.of(mapper.mapRow(friendResultSet(), 0));
                });

        List<MemoryVectorSearchCandidate> candidates = repository.searchFriend(
                OWNER_ID,
                FRIEND_ID,
                List.of(0.1d, 0.2d),
                "mock",
        ...
        assertThat(sql)
                ...
                .contains("f.least_user_id = ?")
                .contains("f.greatest_user_id = ?")
                .contains("e.embedding <=> ?::vector")
                .contains("order by score asc");
    }

    @Test
    void searchFriendRestrictsCandidatesToAcceptedFriendAndActiveSucceededEmbeddings() throws Exception {
        JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
        JdbcMemoryVectorSearchRepository repository = new JdbcMemoryVectorSearchRepository(jdbcTemplate);
        when(jdbcTemplate.query(
                        anyString(),
    ...
    private ResultSet friendResultSet() throws Exception {
        ResultSet rs = mock(ResultSet.class);
        when(rs.getObject("post_id", UUID.class)).thenReturn(POST_ID);
        when(rs.getObject("chunk_id", UUID.class)).thenReturn(CHUNK_ID);
        when(rs.getObject("owner_id", UUID.class)).thenReturn(FRIEND_ID);
        when(rs.getString("owner_nickname")).thenReturn("friend");
        when(rs.getString("title")).thenReturn("Retrospective");
        when(rs.getString("snippet")).thenReturn("JWT Bearer decision");
        when(rs.getString("source_kind")).thenReturn("post_content");
        when(rs.getDouble("score")).thenReturn(0.82d);
